...
if __name__ == '__main__':
    ...
    pkl_output_folder = './data/ryks/Ego_pkl_csv/'
    # create a folder to store the pkl file
    if not os.path.exists(pkl_output_folder):
        os.makedirs(pkl_output_folder)
    # get the train list and val list
    all_list =[]
    ...
    train_dataset = {'rgb':[], 'depth':[],'label':[]}
    val_dataset = {'rgb':[], 'depth':[],'label':[]}
    ...
    for folder in os.listdir(root_frame_path):
        ...
        #make a list that store every frame path in os.path.join(root_frame_path,"rgb")
            ...
            for frame in os.listdir(os.path.join(root_frame_path, folder, "dep")):
                depth_frame_list.append(os.path.join(root_frame_path, folder, "dep", frame))
            label = int(folder.split("_")[-1])
            train_dataset['rgb'].append(rgb_frame_list)
            train_dataset['depth'].append(depth_frame_list)
            ...
            for frame in os.listdir(os.path.join(root_frame_path, folder, "dep")):
                depth_frame_list.append(os.path.join(root_frame_path, folder, "dep", frame))
            label = int(folder.split("_")[-1])
            val_dataset['rgb'].append(rgb_frame_list)
            val_dataset['depth'].append(depth_frame_list)
